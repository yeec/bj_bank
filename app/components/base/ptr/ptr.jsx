...
class PullToRefresh extends Component {
    ...
    loaderDefaultIcon: progress => {
      ...
      return (
        <div style={{ flex: 1, padding: "5px" }}>
          <Icon
            name={progress !== 100 ? "download" : "success"}
            style={style}
          />
        </div>
      );
  ...
  render() {
    ...
    let cls = classNames("react-ryt-ptr", className);
    ...
    let loaderStyle = {
      ...
      height: loaderHeight,
      marginTop: `${-loaderHeight +
        this.state.pullPercent / (100 / loaderHeight)}px`,
      transition: this.state.animating ? "all .5s" : "none"
    };

    return (
      <div className={cls} style={containerStyle} {...domProps}>
        <div className="react-ryt-ptr-loader" style={loaderStyle}>
          {this.state.loading
            ? loaderLoadingIcon
            : loaderDefaultIcon(this.state.pullPercent)}
        </div>
        <div
          className="react-ryt-ptr-content"
          ref="content"
